...
impl Jieba {
    ...
    fn cut<'a>(&self, text: &'a str, hmm: bool) -> Vec<&'a str> {
        self.jieba.cut(text, hmm)
    }

    /// Cut the input text, return all possible words
    #[text_signature = "($self, text)"]
    fn cut_all<'a>(&self, text: &'a str) -> Vec<&'a str> {
        self.jieba.cut_all(text)
    }

    /// Cut the input text in search mode
    #[args(hmm = "true")]
    #[text_signature = "($self, text, hmm)"]
    fn cut_for_search<'a>(&self, text: &'a str, hmm: bool) -> Vec<&'a str> {
        self.jieba.cut_for_search(text, hmm)
    }

    /// Tag the input text
    #[args(hmm = "true")]
    #[text_signature = "($self, text, hmm)"]
    fn tag<'a>(&'a self, text: &'a str, hmm: bool) -> Vec<(&'a str, &'a str)> {
        self.jieba
            .tag(text, hmm)
            .into_iter()
            .map(|t| (t.word, t.tag))
    ...
    fn tokenize<'a>(&self, text: &'a str, mode: &str, hmm: bool) -> Vec<(&'a str, usize, usize)> {
